...
class Veteriner {
  ...
  Veteriner({required this.ad, required this.latitude, required this.longitude});
}
...
class _acilVeterinerlerState extends State<acilVeterinerler> {
  Completer<GoogleMapController> _haritaController = Completer<GoogleMapController>();
  late LocationData _konum;
  List<Veteriner> _veterinerler = [
    Veteriner(ad: "Vena Hayvan Hastanesi", latitude: 39.990931969463574,longitude: 32.642396683962005),
    Veteriner(ad: "Balgat Veteriner Kliniği",latitude: 39.897981,longitude: 32.818801),
    Veteriner(ad: "Dışkapı Veteriner Merkezi Kliniği",latitude: 39.956733,longitude: 32.863493),
    Veteriner(ad: "ATAKULE VETERİNER POLİKLİNİĞİ",latitude: 39.87812542489598,longitude: 32.86524299856006),
    Veteriner(ad: "Mor Pati Veteriner Kliniği",latitude: 39.87450151327733,longitude: 32.68925108160646),
    Veteriner(ad: "Başkent Hayvan Hastanesi ",latitude: 39.89511415623712,longitude: 32.85008802578526),
  ];
        ...
        if (mesafe <= 5000) {
          _markerler.add(
            Marker(
              ...
              infoWindow: InfoWindow(title: veteriner.ad, snippet: 'Mesafe: $mesafe metre'),
            ),
          );
        }
      });
  ...
  double _hesaplaMesafe(double lat1, double lon1, double lat2, double lon2) {
    ...
    var a = math.sin(dLat / 2) * math.sin(dLat / 2) +
        math.cos(_dereceToRadyan(lat1)) * math.cos(_dereceToRadyan(lat2)) * math.sin(dLon / 2) * math.sin(dLon / 2);
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          leading: IconButton(
            ...
            Navigator.of(context).pop();} // BURAYA ANA MENUYE GIDEN YONLENDIRMEYI YAPPPPPPPPPPP
          ),
      ),
      body: GoogleMap(
